...
Use direct read, grep, find, and Bash inspection when the target is already known. Use `oracle_research` only for a focused, multi-step codebase discovery question whose intermediate search would distract from the review or decision. Give the Explore subagent a self-contained brief with concrete success criteria and request file-and-line evidence. You remain responsible for interpreting its findings: do not delegate final judgment, implementation, or another Oracle review, and do not duplicate its searches while it runs.
